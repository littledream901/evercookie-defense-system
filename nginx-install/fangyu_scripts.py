...
class NginxConfigGenerator:
    ...
    @staticmethod
    def generate_config_blocks(domain: str, site_id: str, app_id: str, 
                              app_secret: str, gateway_url: str) -> Tuple[str, str, str]:
        """生成 Nginx 配置块"""
        domain = _clean_nginx_value(domain)
        site_id = _clean_nginx_value(site_id)
        app_id = _clean_nginx_value(app_id)
        app_secret = _clean_nginx_value(app_secret)
...
class NginxConfigManager:
    ...
    def update_website_config(self, domain: str, site_id: str, app_id: str, 
                            app_secret: str, gateway_url: str, container_id: str,
        ...
        modified_config = self._modify_config(
            current_config, config_domain, site_id, app_id, app_secret, gateway_url
        )
    ...
    def _modify_config(self, config: str, domain: str, site_id: str, 
                      app_id: str, app_secret: str, gateway_url: str) -> str:
        """修改配置内容"""
    ...
    def _test_nginx_config(self, container_id: str, config_path: str) -> Optional[bool]:
        ...
        try:
            ...
            checks = {
                ...
                "fangyu_app_id": "fangyu_app_id" in config,
                "fangyu_app_secret": "fangyu_app_secret" in config,
...
class FangyuInstaller:
    ...
    def install(self, domain: str, site_id: str, app_id: str, 
               app_secret: str, gateway_url: str, skip_ssl_check: bool = False) -> bool:
        """执行完整的安装流程"""
        ...
        try:
            ...
            config_path = self.nginx_manager.update_website_config(
                domain, site_id, app_id, app_secret, gateway_url, container_id, skip_ssl_check
            )
...
def main(domain: str, site_id: str, app_id: str, app_secret: str, 
...
if __name__ == "__main__":
    ...
    SITE_ID = "site_eba8689a"         # 防御系统后台的 Site ID
    APP_ID = "1"                     # 防御系统后台的 App ID（数字）
    APP_SECRET = "bd5f8a076002101ff410fd127dd5d5e71452c00e9aa479bf" # 防御系统后台的 App Secret
    ...
    # 检查配置是否填写
    if DOMAIN == "example.com" or SITE_ID == "site_abc123" or APP_SECRET == "your_secret_here":
        print(f"{Colors.RED}❌ 请先在脚本底部的配置区填写正确的配置！{Colors.END}")
        print(f"当前配置:")
        print(f"  DOMAIN = {DOMAIN}")
        print(f"  SITE_ID = {SITE_ID}")
        print(f"  APP_SECRET = {APP_SECRET[:20]}...")
        sys.exit(1)
    ...
    main(DOMAIN, SITE_ID, APP_ID, APP_SECRET, GATEWAY_URL, PANEL_URL, PANEL_KEY, SKIP_SSL_CHECK)
