...
def _render_blocks(domain: str, site_id: str, app_id: str, app_secret: str, gateway_url: str) -> Tuple[str, str, str]:
    domain = _clean_value(domain)
    site_id = _clean_value(site_id)
    app_id = _clean_value(app_id)
    app_secret = _clean_value(app_secret)
    ...
    set $fangyu_app_id       "{app_id}";
    set $fangyu_app_secret   "{app_secret}";
...
class FangyuTemplateMigrator:
    ...
    @staticmethod
    def migrate_config(config_content: str, site_id: str, app_id: str, app_secret: str, gateway_url: str) -> str:
        """
        迁移 Nginx 配置为 Fangyu Defense 模板配置。
        
        Args:
            config_content: 原始 Nginx 配置内容
            site_id: Fangyu 站点 ID
            app_id: Fangyu 应用 ID
            app_secret: Fangyu 应用密钥
        ...
        vars_block, access_lua, body_filter = _render_blocks(
            domain, site_id, app_id, app_secret, gateway_url
        )
...
class NginxConfigGenerator:
    @staticmethod
    def generate_config_blocks(domain: str, site_id: str, app_id: str, app_secret: str, gateway_url: str) -> Tuple[str, str, str]:
        domain = _clean_value(domain)
        site_id = _clean_value(site_id)
        app_id = _clean_value(app_id)
        app_secret = _clean_value(app_secret)
...
class NginxConfigManager:
    ...
    def update_website_config(self, domain: str, site_id: str, app_id: str, app_secret: str, gateway_url: str, container_id: str, skip_ssl_check: bool = False) -> str:
        """
        更新站点 Nginx 配置（带逻辑验证）。
        ...
        modified_config = self._modify_config(current_config, config_domain, site_id, app_id, app_secret, gateway_url)
    ...
    def _modify_config(self, config: str, domain: str, site_id: str, app_id: str, app_secret: str, gateway_url: str) -> str:
        vars_block, access_lua, body_filter = self.config_generator.generate_config_blocks(domain, site_id, app_id, app_secret, gateway_url)
...
class FangyuInstaller:
    ...
    def install(self, domain: str, site_id: str, app_id: str, app_secret: str, gateway_url: str, skip_ssl_check: bool = False) -> bool:
        try:
            ...
            config_path = self.nginx_manager.update_website_config(domain, site_id, app_id, app_secret, gateway_url, container_id, skip_ssl_check)
...
def install_main(domain: str, site_id: str, app_id: str, app_secret: str, gateway_url: str, panel_url: str, panel_key: str, skip_ssl_check: bool = False) -> None:
    """
    安装器主入口函数（被 CLI 调用）。
    ...
    success = installer.install(domain, site_id, app_id, app_secret, gateway_url, skip_ssl_check)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        SITE_ID = "site_eba8689a"
        APP_ID = "1"
        APP_SECRET = "bd5f8a076002101ff410fd127dd5d5e71452c00e9aa479bf"
