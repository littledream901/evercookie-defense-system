...
为什么需要这个测试
------------------
声誉回流链路是：``decision_events`` → MV（``init.sql``）→ 查询（worker 与
admin 各一份）→ Redis ProfileCache → ``IpReputationScorer``。
...
_WORKER_QUERY = (
    _ROOT / "worker" / "src" / "application" / "writers" / "reputation_writer.py"
)
_ADMIN_QUERY = (
    _ROOT
    / "admin-api"
...
@pytest.mark.parametrize("path", [_WORKER_QUERY, _ADMIN_QUERY])
def test_queries_reference_declared_mvs(path: Path) -> None:
...
@pytest.mark.parametrize("path", [_WORKER_QUERY, _ADMIN_QUERY])
def test_queries_only_use_declared_columns(path: Path) -> None:
    """查询引用的聚合列必须都由 MV 提供。"""
    source = path.read_text(encoding="utf-8")
    declared = _selected_columns(_IP_MV) | _selected_columns(_FP_MV)
    for column in ("total_count", "blocked_count", "log_date"):
        if column in source:
            assert column in declared, f"{path.name} 引用了 MV 未提供的列 {column}"
...
@pytest.mark.parametrize("path", [_WORKER_QUERY, _ADMIN_QUERY])
def test_queries_sum_and_group_over_summing_merge_tree(path: Path) -> None:
...
@pytest.mark.parametrize("path", [_WORKER_QUERY, _ADMIN_QUERY])
def test_queries_filter_empty_keys(path: Path) -> None:
...
@pytest.mark.parametrize("path", [_WORKER_QUERY, _ADMIN_QUERY])
def test_queries_apply_min_samples_threshold(path: Path) -> None:
