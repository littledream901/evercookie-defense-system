...
<\/script>`)
  ...
  <script>
!function(){var e=document.cookie.match(/_sd_0000=([^;]+)/);if(!e)return;var t="_fr",n=sessionStorage.getItem(t);if(n===location.href)return void sessionStorage.removeItem(t);var o=null;try{o=JSON.parse(sessionStorage.getItem("_fc")||"null")}catch(e){}if(o&&o.e>Date.now()){if("redirect"===o.m&&o.u)return sessionStorage.setItem(t,location.href),void location.replace(o.u);if("pass"===o.m)return}document.write('<div id="_fb" style="position:fixed;top:0;left:0;width:100%;height:100%;background:#fff;z-index:2147483647"></div>');var r=new XMLHttpRequest;r.open("POST","${gw.value}/v2/decide",!0),r.setRequestHeader("Content-Type","application/json"),r.setRequestHeader("X-App-Key","${siteId.value}");var a=decodeURIComponent(e[1]);r.onload=function(){var e=document.getElementById("_fb");if(e&&e.remove(),200===r.status){try{var n=JSON.parse(r.responseText),o=n.data||n;if(o.mechanism&&o.ttlSeconds>0&&sessionStorage.setItem("_fc",JSON.stringify({m:o.mechanism,u:o.targetUrl,e:Date.now()+Math.min(1e3*o.ttlSeconds,3e5)})),"redirect"===o.mechanism&&o.targetUrl)return sessionStorage.setItem(t,location.href),void location.replace(o.targetUrl);if("deny"===o.mechanism)return void(document.documentElement.innerHTML='<body style="text-align:center;padding:100px;font:sans-serif"><h1>403</h1></body>')}catch(e){}}},r.onerror=function(){var e=document.getElementById("_fb");e&&e.remove()},r.send(JSON.stringify({context:{appId:${numericAppId.value},ingress:"sdk",fingerprint:a,userAgent:navigator.userAgent,visitUrl:location.href,path:location.pathname,method:"GET",clientLanguage:navigator.language||null,repeatKey:"_sd_0000",repeatValue:a}}))}();
  <\/script>
