...
class AppKeyRedisSync:
    ...
    async def bind(self, api_key: str, app_id: int) -> None:
        if not api_key or app_id <= 0:
            return
        try:
            if self._ttl:
                await self._redis.set(self._redis_key(api_key), str(app_id), ex=self._ttl)
            else:
                await self._redis.set(self._redis_key(api_key), str(app_id))
    ...
    async def rebind(self, old_key: str | None, new_key: str, app_id: int) -> None:
        if old_key and old_key != new_key:
            await self.unbind(old_key)
        await self.bind(new_key, app_id)
